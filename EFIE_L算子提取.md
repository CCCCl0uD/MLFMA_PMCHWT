# EFIE 中 L 算子的公式与代码映射及完整提取

## 1. 范围

本文仅分析并提取 `Code/EFIE.h` 与 `Code/EFIE.cpp` 中的 EFIE/L 算子代码，公式依据为 `任务1.md`，不展开其他代码文件。

## 2. 公式与伽辽金离散

```math
\mathcal L(\mathbf X)=\mathrm j k_0\iint_{S_0}
\left[\mathbf X(\mathbf r')G_0+
\frac{1}{k_0^2}(\nabla'\!\cdot\mathbf X)\nabla G_0\right]\mathrm dS',
\qquad
G_0=\frac{e^{-\mathrm j k_0R}}{4\pi R}.
```

使用 RWG 基函数和伽辽金测试，并对标量势项分部积分后，代码对应的矩阵元为

```math
Z_{mn}^{(L)}
=\mathrm j k_0\eta_0\iint f_m\cdot f_nG_0\,\mathrm dS'\mathrm dS
-\frac{\mathrm j\eta_0}{k_0}\iint
(\nabla\!\cdot f_m)(\nabla'\!\cdot f_n)G_0\,\mathrm dS'\mathrm dS.
```

每个 RWG 支撑三角形上有

```math
f(\mathbf r)=s\frac{l}{2A}(\mathbf r-\mathbf r_0),\qquad
\nabla\cdot f=s\frac{l}{A},\qquad s\in\{-1,+1\}.
```

代码先计算不含边长和符号的局部量 `(r-r0)/(2A)`，最终由
`(lF*lS)/(4*Pi)*signSymbolF*signSymbolS`
统一乘入边长、RWG 正负号和格林函数的 `1/(4π)`。所以局部 `G` 使用
`exp(-J*k_*R)/R`，此处不再除以 `4*Pi`。

## 3. 公式与代码映射

| 数学对象/步骤 | 代码实现 |
|---|---|
| L 算子矩阵元入口 | `EFIE::computeEFIE_Zij` |
| 场/源 RWG 的正负支撑三角形 | `i=0..1`、`j=0..1` |
| `1/(2A)` 与局部 RWG 向量 | `constRWGField/Source`、`RWGf/RWGs` |
| RWG 边长与正负号 | `lF/lS`、`signSymbolF/signSymbolS` |
| 双重面积积分 | `numI`、`numJ` 两层高斯积分 |
| 格林核 | `G=exp(-J*k_*R)/R` |
| 矢量势项 | `J*k_*eta_*G*dot(RWGf,RWGs)` |
| 分部积分后的标量势项 | `-(J*eta_/k_)*G/(areaTriF*areaTriS)` |
| `1/(4π)`、边长、正负号 | 最终累加 `Zij_near_E` |
| 同面 `1/R` 奇异补偿 | `EFIE::singularityEFIE` |
| 伽辽金测试 | `singularityEFIE` 末尾以 `RWGf` 测试 |
...
```math
\frac{e^{-\mathrm jkR}}R
=\frac{e^{-\mathrm jkR}-1}R+\frac1R.
```

平滑余项由 `G_Sing` 高斯积分，`R→0` 时取极限 `-jk`；`1/R` 项由
`singularityEFIE` 解析计算并通过 `alok` 加回。
...
`computeEFIE_Zij` 负责矩阵元装配与奇异核拆分，`singularityEFIE` 负责同面奇异积分补偿。
...
- `computeL_Zij`：遍历场/源 RWG 的四组三角形组合，完成矢量势项、标量势项、伽辽金双重积分和矩阵元累加。
- `singularityL`：场、源三角形相同时，对拆出的 `1/R` 奇异核进行解析补偿。

提取仅调整函数命名及命名空间，计算流程、RWG 正负号、面积因子、`1/(4π)` 归一化和奇异项算法均保持不变。

| 原 EFIE 名称 | 提取后的 L 算子名称 |
|---|---|
| `namespace EFIE` | `namespace LOperator` |
| `computeEFIE_Zij` | `computeL_Zij` |
| `singularityEFIE` | `singularityL` |
| `Zij_near_E` | `Zij_near_L` |
...
namespace LOperator {
	void computeL_Zij(const RWGBase* rwgField, const RWGBase* rwgSource,
		const std::complex<double> k_, const std::complex<double> eta_,
		const gaussPoints& gp, std::complex<double>& Zij_near_L);

	void singularityL(double& areaTriF, double& areaTriS,
		...
		double* nonPublicEdge_vertexS, const double recAs,
		double* normalVectorS, const gaussPoints& gp,
		const std::complex<double> k_, const std::complex<double> eta_,
...
namespace LOperator {
	void computeL_Zij(const RWGBase* rwgField, const RWGBase* rwgSource, const std::complex<double> k_, const std::complex<double> eta_,
		const gaussPoints& gp, std::complex<double>& Zij_near_L) {
		int rwgFieldTriId, rwgSourceTriId;
		double areaTriF, areaTriS;
		double constRWGField, constRWGSource;
		...
		for (int i = 0; i < 2; ++i) {
			...
			for (int j = 0; j < 2; ++j) {
				...
				if (rwgFieldTriId != rwgSourceTriId) {
					...
					singularityL(areaTriF, areaTriS,
						vertexF1, vertexF2, vertexF3, nonPublicEdge_vertexF, constRWGField,
						vertexS1, vertexS2, vertexS3, nonPublicEdge_vertexS, constRWGSource, normalVectorS,
						gp, k_, eta_, alok);
					alok1 += alok;
				...
				const int signSymbolF = (i == 0) ? -1 : 1;
				const int signSymbolS = (j == 0) ? -1 : 1;
				Zij_near_L += (lF * lS) / (4.0 * Pi) * double(signSymbolF * signSymbolS) * alok1;
			}
		}
	}
