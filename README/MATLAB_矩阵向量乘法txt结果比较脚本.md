# MATLAB 矩阵向量乘法 txt 结果比较脚本

## 功能

用于任选两个矩阵向量乘法导出的 `txt` 文件，比较其中三列：
...
1. 两个文件的向量长度是否一致。
2. `y = y_real + 1i * y_imag` 的最大绝对误差。
3. `y` 的相对 2 范数误差。
4. `y_real`、`y_imag`、`abs_y` 三列各自的最大误差和相对误差。
5. 误差最大的若干行。
6. 可选误差曲线图。
...
```matlab
yA = yA_real + 1i * yA_imag
yB = yB_real + 1i * yB_imag
diffY = yB - yA
```
...
```matlab
max(abs(diffY))
norm(diffY, 2) / norm(yA, 2)
```

其中 `yA` 被作为参考文件。脚本也会单独比较 `y_real`、`y_imag`、`abs_y` 三列，方便定位误差来自实部、虚部还是幅值列。

## 注意事项

1. 两个 txt 文件必须使用当前导出格式：`index x_real x_imag y_real y_imag abs_y`。
2. 两个文件行数必须一致。
3. 如果 `index` 列不一致，脚本会警告，但仍按行顺序比较。
4. 若希望关闭绘图，把脚本开头改为：
