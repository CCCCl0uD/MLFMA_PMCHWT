...
namespace OLK {
	...
	inline void singK(double& areaTriF, double& areaTriS,
	...
	inline void K_operator(const RWGBase* rwgField, const RWGBase* rwgSource, const std::complex<double> k_,
		const gaussPoints& gp, std::complex<double>& Zij_near_M)
	{
		...
		for (int fi = 0; fi < 2; ++fi) {
			...
			for (int sj = 0; sj < 2; ++sj) {
				...
				Zij_near_M += signSymbolF * signSymbolS * (lF * lS) / (4.0 * Pi) * alok1;
			}
		}
	}
}
